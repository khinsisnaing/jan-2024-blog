...
if(isset($_POST['article-update'])) {
    ...
    if(!$error) {
        ...
        if(is_uploaded_file($photos['tmp_name'][0])) {
            ...
            $query = "UPDATE article SET title='$title', body='$body', photos='$photo_path', category_id='$category_id' WHERE id='$id'";
        } else {
            $query = "UPDATE article SET title='$title', body='$body', category_id='$category_id' WHERE id='$id'";
        }
